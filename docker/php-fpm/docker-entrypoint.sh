...
# Loop through XDEBUG, PHP_IDE_CONFIG and XDEBUG_CLIENT_HOST variables and check if they are set.
...
for VAR in XDEBUG PHP_IDE_CONFIG XDEBUG_CLIENT_HOST XDEBUG_CLIENT_PORT XDEBUG_PHP_IDE_KEY
do
...
# If there is still no value for the XDEBUG_CLIENT_HOST variable then we set it to the default of host.docker.internal. This
# value will be sufficient for windows and mac environments.
if [ -z "${XDEBUG_CLIENT_HOST}" ]; then
  XDEBUG_CLIENT_HOST="host.docker.internal"
  sed -i "/XDEBUG_CLIENT_HOST/d"  ~/.bashrc
  echo "export XDEBUG_CLIENT_HOST=\"$XDEBUG_CLIENT_HOST\"" >> ~/.bashrc;
fi
...
if [ "true" == "$XDEBUG" ] && [ ! -f /usr/local/etc/php/conf.d/docker-php-ext-xdebug.ini ]; then
  ...
  echo "xdebug.idekey=$XDEBUG_PHP_IDE_KEY" >> /usr/local/etc/php/conf.d/docker-php-ext-xdebug.ini; \
